...
#==
function HyDe_ind(outgroup::String, p::Phylip, 
                    pval::Float64, 
                    display_all::Bool, #filter
                    writecsv::Bool, 
                    filename::AbstractString)
    ...
    for n in 1:length(p.allquartet)
        if p.allquartet[n][1]==ourgroup_id
            ...
            t3=p.nametaxa[p.allquartet[n][3]]
            t4=p.nametaxa[p.allquartet[n][4]]
            #println(typeof(t1),typeof(t2),typeof(t3),typeof(t4))
            #println(t1,t2,t3,t4)
            push!(data,[p.allquartet[n][1],p.allquartet[n][2],p.allquartet[n][3],p.allquartet[n][4],
                        p.spcounts[n][4],p.spcounts[n][7],p.spcounts[n][9],t1,t2,t3,t4])
        end
    end

    #HyDe analysis
    for n in 1:length(data)
        p9 = (data[n][7]+0.05)/num_obs #ABBA
        p7 = (data[n][6]+0.05)/num_obs #ABAB
        p4 = (data[n][5]+0.05)/num_obs #AABB
        ...
        if display_all
            if p_val<(pval/3)
                #if p_val<=(pval) ast="*" else ast="" end
                ast="*"
                #println("$newQ, gamma: $gamma, z-score: $GH_ts, p-val: $p_val")
                push!(res,[data[n][8],data[n][9],data[n][10],data[n][11],
                        data[n][5],data[n][6],data[n][7],gamma,GH_ts,p_val,ast])
            else
                ast=""
                #println("$newQ, gamma: $gamma, z-score: $GH_ts, p-val: $p_val")
                push!(res,[data[n][8],data[n][9],data[n][10],data[n][11],
    ...
    #write csv
    if filename=="" filename="HyDe-out" end
    if (writecsv)
        CSV.write("$filename.csv",df) 
    end
...
function HyDe_map(outgroup::String, 
                p::Phylip, 
    ...
    for n in 1:length(data)
        ...
        t4count=0
        for i in 1:size(mapd,1)
            if mapd[i,2]==data[n][8]
                t1count+=1
            elseif mapd[i,2]==data[n][9]
                t2count+=1
            elseif mapd[i,2]==data[n][10]
                t3count+=1
            elseif mapd[i,2]==data[n][11]
                t4count+=1
            end
        end                                
        
        avg_obs=num_obs/(t1count*t2count*t3count*t4count)#(self.outIndex.shape[0] * p1.shape[0] * hyb.shape[0] * p2.shape[0])
        avobs=avg_obs
